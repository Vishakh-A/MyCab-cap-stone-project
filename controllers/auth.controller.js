...
module.exports.adminLogin =async(req,res)=>{
    ...
    jwt.sign({adminId:findAdmin._id,username:findAdmin.username},admin_secret,(err,token)=>{
        if(err){
            console.log(err);
            return res.status(200).json({message:'error'});
        }
...
module.exports.userLogin =async(req,res)=>{
    ...
    jwt.sign({userid:findUser._id,username:findUser.username},user_secret,(err,token)=>{
        if(err){
            console.log(err);
            return res.status(200).json({message:'error'});
        }
